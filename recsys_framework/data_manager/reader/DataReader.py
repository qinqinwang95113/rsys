    ...
    def load_data(self, save_folder_path=None, postprocessings=None):
        ...
        if save_folder_path is None:
            ...
            for postprocessing in postprocessings:
                if os.path.exists(progressive_save_folder_path + postprocessing.get_name()) and \
                        self.all_files_available(progressive_save_folder_path + postprocessing.get_name()):
        ...
        if not self.reload_from_original_data:
        ...
        for i in range(postprocessing_index, len(postprocessings)):
            print("reader: Applying {}".format(postprocessings[i]))
            dataset = postprocessings[i].apply(dataset)
