...
class Dataset(object):
    ...
    def remove_item_features(self, min_occurrence=5, max_perc_occurrence=0.3):

        for key in self.ICM_dict.keys():
            self.ICM_dict[key], _, self.ICM_mappers_dict[key] = removeFeatures(self.ICM_dict[key],
                                                                               minOccurrence=min_occurrence,
                                                                               maxPercOccurrence=max_perc_occurrence,
                                                                               reconcile_mapper=self.ICM_mappers_dict[key])


    def remove_user_features(self, min_occurrence=5, max_perc_occurrence=0.3):

        for key in self.UCM_dict.keys():
            self.UCM_dict[key], _, self.UCM_mappers_dict[key] = removeFeatures(self.UCM_dict[key],
                                                                               minOccurrence=min_occurrence,
                                                                               maxPercOccurrence=max_perc_occurrence,
                                                                               reconcile_mapper=self.UCM_mappers_dict[key])
