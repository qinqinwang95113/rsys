...
from recsys_framework.utils import seconds_to_biggest_unit
import os
    ...
    def _train_with_early_stopping(self, epochs_max, epochs_min=0,
        ...
        while epochs_current < epochs_max and not convergence:
            ...
            # Determine whether a validaton step is required
                ...
                metrics_handler = evaluator_object.evaluateRecommender(self)
                results_run = metrics_handler.get_results_dictionary()
                results_run_string = metrics_handler.get_results_string()

                results_run = results_run[list(results_run.keys())[0]]

                # save in a dictionary the partial result of the run
                self._epoch_result_dictionary[epochs_current+1]=results_run

                print("{}:\n{}".format(algorithm_name, results_run_string))
                ...
                if stop_on_validation and lower_validatons_count >= lower_validations_allowed and epochs_current >= epochs_min:
                    ...
                    new_time_value_total, new_time_unit_total = seconds_to_biggest_unit(elapsed_time_total)

                    elapsed_time_training = elapsed_time_total - validation_time
                    new_time_value_training, new_time_unit_training = seconds_to_biggest_unit(elapsed_time_training)

                    print("{}: Convergence reached! Terminating at epoch {}. Best value for '{}' at epoch {} is {:.4f}. Elapsed time total {:.2f} {}. Elapsed time training {:.2f} {}".format(
                        algorithm_name, epochs_current+1, validation_metric, self.epochs_best, self.best_validation_metric, new_time_value_total, new_time_unit_total, new_time_value_training, new_time_unit_training))

                    self._elapsed_time_total = elapsed_time_total
                    self._elapsed_time_training = elapsed_time_training
            ...
            if self.verbose:
                print("{}: Epoch {} of {}. Elapsed time total {:.2f} {}. Elapsed time training {:.2f} {}".format(
                    algorithm_name, epochs_current+1, epochs_max, new_time_value_total, new_time_unit_total, new_time_value_training, new_time_unit_training))
        ...
        if not convergence:
            ...
            if evaluator_object is not None and self.best_validation_metric is not None:
                print("{}: Terminating at epoch {}. Best value for '{}' at epoch {} is {:.4f}. Elapsed time total {:.2f} {}. Elapsed time training {:.2f} {}".format(
                    algorithm_name, epochs_current, validation_metric, self.epochs_best, self.best_validation_metric, new_time_value_total, new_time_unit_total, new_time_value_training, new_time_unit_training))
            else:
                print("{}: Terminating at epoch {}. Elapsed time total {:.2f} {}. Elapsed time training {:.2f} {}".format(
                    algorithm_name, epochs_current, new_time_value_total, new_time_unit_total, new_time_value_training, new_time_unit_training))

            self._elapsed_time_total = elapsed_time_total
            self._elapsed_time_training = elapsed_time_training

    def plot_metric_history(self, metric, title='', fontsize=15, save=False, show=True):
        """
        Plot the metric every validation_every_n epochs to see the behaviour in time
        :param metric: list of metrics to print
        :return:
        """

        assert len(self._epoch_result_dictionary.keys())>0, 'Model has not been trained !'
        assert metric in self._epoch_result_dictionary[self.validation_every_n].keys(), 'selected metric is not valid!'

        x=[]
        y=[]
        for key in self._epoch_result_dictionary.keys():
            x.append(key)
            y.append(self._epoch_result_dictionary[key][metric])
        ...
        if save:
            # save the fig 
            if not os.path.exists('EarlyStoppingPlots/'):
                os.makedirs('EarlyStoppingPlots/')
            n = 0
            while os.path.isfile('EarlyStoppingPlots/{}_{}_{}.png'.format(self.algorithm_name, metric, n)):
                n += 1
            file_name = 'EarlyStoppingPlots/{}_{}_{}.png'.format(self.algorithm_name, metric, n)
            print('saving the figure in {}'.format(file_name))
