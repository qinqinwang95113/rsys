...
import numpy as np
from recsys_framework.recommender.data_io import DataIO
from recsys_framework.utils import check_matrix
from tqdm import tqdm
import os
from recsys_framework.utils.early_stopping import EarlyStoppingModel
import datetime
from recsys_framework.utils.write_textual_file import WriteTextualFile
from recsys_framework.utils.log import dict_to_string
...
class Recommender(object):
    ...
    def _get_dict_to_save(self):
        raise NotImplementedError("recommender: _get_dict_to_save not implemented")


    def save_model(self, folder_path, file_name = None, save_description=True):
        if file_name is None:
            file_name = '{}_{}'.format(self.RECOMMENDER_NAME, datetime.datetime.now().strftime("%Y-%m-%d_%H:%M:%S")) 

        self._print("Saving model in file '{}'".format(folder_path + file_name))

        if hasattr(self, 'model_parameters') and save_description:
            wr = WriteTextualFile(folder_path, file_name)
            wr.write_line(dict_to_string(self.model_parameters, style='constructor'))

        data_dict_to_save = self._get_dict_to_save()

        # in case I'm training with early stopping, save also the
        #  performance of the model through the epochs
        if isinstance(self, EarlyStoppingModel):
            data_dict_to_save["_epoch_result_dictionary"] = self._epoch_result_dictionary
            data_dict_to_save["_elapsed_time_total"] = self._elapsed_time_total
            data_dict_to_save["_elapsed_time_training"] = self._elapsed_time_training
