...
from recsys_framework.utils.compute_popularity import compute_popularity_item, compute_popularity_user
...
class EvaluatorUsersByPopularity(Evaluator):
    ...
    def _run_evaluation_on_selected_users(self, recommender_object, block_size=None):
        ...
        for t in zip(self.thr,self.thr[1:]):
            ignore_users_dict[t] = users[(cum_sum_interactions >= t[0]*tot_interactions) & (cum_sum_interactions <= t[1]*tot_interactions)]
            keep_users_dict[t] = np.array(list(set(users) - set(ignore_users_dict[t])))
    ...
    def evaluateRecommender(self, recommender_object, URM_test=None, URM_test_negative=None, ignore_users=None, ignore_items=None):
        ...
        for test_user in self.usersToEvaluate:
            ...
            for cutoff in self.cutoff_list:
                ...
                results_current_cutoff[EvaluatorMetrics.COVERAGE_ITEM_TEST.value].add_recommendations(recommended_items_current_cutoff, is_relevant_current_cutoff)
